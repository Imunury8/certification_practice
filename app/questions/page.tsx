...
export default function QuestionsPage() {
  const [selectedTopic, setSelectedTopic] = useState<TopicId>("design-patterns");
  const [difficulty, setDifficulty] = useState<Difficulty>("medium");
  const [count, setCount] = useState("8");
  const [focus, setFocus] = useState("");
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [questions, setQuestions] = useState<Question[]>(() =>
    generateQuestions({ topicId: "design-patterns", difficulty: "medium", count: 8 }),
  );
  ...
  function handleExport() {
    ...
    anchor.href = url;
    anchor.download = "정보처리기사-실기-문제.txt";
    anchor.click();
        ...
        <section className="workspace">
          <aside className="panel controls" aria-label="문제 생성 설정">
              ...
              <div className="topic-grid">
                {TOPICS.map((topic) => (
                  ...
                  >
                    ...
                    <small>{topic.keywords.length}개 키워드</small>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="architecture">
              현재는 내장 문제 은행을 사용합니다. 이후 `/app/api/generate/route.ts`의 provider를 AI 모델로 교체하면
              같은 풀이 화면에서 생성형 문제를 받을 수 있습니다.
            </div>
          </aside>
